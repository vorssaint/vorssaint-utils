...
- Dynamic Island can show each new song for a moment, with its cover, title and artist beside the camera, with New track under Feedback in its settings. It is off by default, and a click opens the music section. Thanks to @Yahddyyp.
...
- Dynamic Island stays on screen on a display without a camera cutout when the menu bar hides automatically, instead of disappearing when an app opens and coming back when it closes. Thanks to @Yahddyyp.
...
- The Shelf stays out of tiling window managers, which listed it as a window on the current desktop while you dragged selected text. Thanks to @Yahddyyp.
