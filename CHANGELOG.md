...
- Dynamic Island opens as a wide strip instead of a tall page: Compact stays about 480 by 290 points and Spacious about 560 by 370, and every page lays itself out sideways inside that height. Home keeps playback and the levels on one row with the shortcuts beneath, the gallery, system cards, tools, clipboard, notifications, downloads, captures and the shelf run sideways when they do not fit, the calendar shows the week as a strip with appointment cards, and the timer names its modes as three quiet words with the current one underlined, Start beside them, with every mode on the same ruler row: the Pomodoro's focus is set on the ruler, its breaks and sessions read as one quiet line of menus beneath, and the stopwatch keeps its clock where the timer's is. The island's mixer is a desk of faders, the output on the left and one per app running sideways, pinned ones first, with everything the panel's list offers: each app's menu pins it, moves it, sends it to another output and hides it, Command-drag reorders, a click on a level types a new one, and a tab at the end holds the sound effects and microphone routing with the panel's options, the hidden apps included. The header shows its actions only under the pointer, keeping a dot for an available update, the gallery's search folds into a magnifier beside its title, and lyrics fill their card, with import behind an icon on the timing row. The Scratchpad joins the island as a page of its own with everything the floating pad does: tabs that close from their own cross, Command-T and Command-W, the editor, formatted reading, export and an undoable clear; its shortcut opens the page whenever it is on. The calendar strip opens a month grid sized to the island. Clipboard cards carry the quick panel's actions, paste, copy, pin, move and delete, with the time of each copy and the recent ones cleared from the search row; the shelf's trash removes the selection first; and the system page adds power and fan cards opening the panel's details. Only the embedded app panel and a metric detail keep scrolling down.
