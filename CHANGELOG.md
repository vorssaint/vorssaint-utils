...
### Summary
Settings lists tools on shared pages by name in the sidebar, and keyboard brightness shortcuts are easier to find. Clipboard History images open in the Screenshot Editor, copied color values show a swatch, the capture preview can pin a screenshot directly, and Settings adds Back and Forward buttons and gives Dock Preview and Dock clicks a Dock page of their own. The Screenshot Editor sets text size apart from line thickness, and blurs in screenshots and recordings can be made lighter or stronger. Window Layout adds quarter rows, quarter columns and vertical thirds, Maximize windows can leave chosen apps with the native full screen button, smooth scrolling gains an adjustable coast, a radial menu wheel can open with a four-finger tap, and the menu panel opens with the native macOS animation. Shelf items can be pinned to stay after use, fan Control can resume your manual speed or curve after a restart or sleep, and the app chosen to open in place of Music starts playing when Play/Pause opened it. Repeating Top Half or Bottom Half moves a window to a stacked display, the menu bar disk reading can show available or used space, App Switcher separates apps without open windows and places apps hidden with Cmd+H like minimized windows, a Command Bar script can run from its own shortcut without opening the bar, Homebrew's Installed list folds dependencies under the packages that need them, and Port Manager marks ports other devices on the network may reach. Dynamic Island can stand in for the menu bar icon, stays in place between desktops, keeps its glass dark as it opens and closes, fits music, connection notices and section search more neatly, and stays open when hovered from a full-screen app on another display. Clipboard History stays responsive with many screenshots, Settings search keeps every typed letter in the search field, Show brightness when adjusting stays off while Dynamic Island is hidden, Extra Brightness holds while a Vorssaint menu or slider is in use, the GPU list names system processes, and Clear all permissions and full uninstall report a refused password request instead of claiming success. Uninstall with Homebrew runs only what its confirmation named, Mic Mute keeps the balance between microphone channels, the Command Bar sets brightness only on the display under the pointer, pinned clipboard items stay when the history outgrows its limit, and a quick paste that cannot reach its app, a settings export that failed, a Super key that macOS refused and an automatic cleanup that left items behind now say so. The menu panel can pick a still wallpaper, Window Layout can move the pointer to the next display, the menu bar Shelf can sit as a badge at the top center of the screen, the Shelf page offers the Dynamic Island or separate window choice, and Command-W closes the screenshot preview. Shelf files in Dynamic Island take the first click and drag out, clicking a window with a sheet on it captures all of it on macOS 27, a brightness key after a pause keeps the level just set on a monitor, the CPU reading after a long pause shows the current load, minimal Dock previews lose their empty bands, and App Switcher no longer refocuses a window that is already in front. Vorssaint speaks Slovak and Ukrainian, Audio device priority keeps ordered output and microphone lists, the menu bar can count connected USB devices, Window Layout can ignore chosen apps and cycle a repeated side through thirds, closed-lid mode can dim the built-in screen, and the keyboard light gets a slider. The App shortcuts table sorts by any column, the Cleaner and the Uninstaller can stop a scan, and shortcuts keep working while the Disk Image Installer asks. Screenshots go straight to AirDrop, Messages and other apps from the preview and the editor, a running timer in Dynamic Island shares the island with the music playing or a working agent, and the island can show each new song for a moment. Dynamic Island stays on screen under a hidden menu bar, shows downloads in narrow wings, keeps playback controls steady as it opens, lets arrow keys pick clipboard search results, keeps Open on notifications from closed apps, steps aside in Mission Control and can count down to the next calendar event, and tiling window managers no longer list Vorssaint's floating panels. Panels, captures and windows dropped from Dock Preview follow the display under the pointer when it rests on the top edge, the Screen capture and Dynamic Island settings fit a narrow window, and an update that asks for your password takes Extra Brightness down first. The Cleaner can list forgotten screenshots, Dynamic Island notices shrink back into the island as they close, lyrics turn up for Apple Music singles and EPs, and the AI section refreshes its totals with a quarter of the CPU.

### Added
- Dynamic Island can show a one-hour countdown with the start time and title of the next timed calendar event in the closed notch. The option is off by default; timers, downloads and working agents keep priority. Thanks to Brain for the feedback.
...
- The screenshot preview and the Screenshot Editor have a Share button that opens the system Share menu, the same one the Shelf uses, to send the screenshot through AirDrop, Messages, Mail or any other app the Mac offers. The link button beside it now reads Temporary links.
...
- Blur strength goes from 1 to 5 for the Screenshot Editor's pixelate areas and for each blur in the Screen Recorder editor. Level 3 keeps the current strength, each area keeps its own level, and a new capture never starts below level 3, since the lighter levels can leave text readable.
...
- A radial menu wheel can open with a four-finger tap on the trackpad, with Open with a four-finger tap in its settings. One wheel has the tap at a time, Middle click keeps a four-finger tap it already uses, and the switch is off by default. Thanks to @PathGao.
...
- A Command Bar script can run from its row's global shortcut without opening the bar when Run from its global shortcut without opening the bar is on in the script's settings. It runs with no argument and beeps when it cannot run or fails. Thanks to @frixaco.
...
- Dynamic Island can stand in for the menu bar icon with Hide the menu bar icon under Behavior in its settings. Settings and the app panel open from the island, menu bar metrics stay, the icon still appears for an available update or the muted microphone indicator, and it returns when Dynamic Island is off. Thanks to DucemMortis for the feedback.
- A running timer in Dynamic Island shares the closed island with the music playing or a working agent. The track's cover or the agent's mark sits left of the camera, as a download already did, the timer stays on the right, and each side opens its own page. A paused or finished timer keeps its own mark there. Thanks to Pinea for the feedback.
- Shelf items can be pinned from their right-click menu. A pinned item stays on the Shelf after you drag it into another app and after Clear all, keeps its pin across restarts, and shows a pin in its corner. Its remove button still takes it away. Thanks to Monolithic capacitor for the feedback.
- The menu panel has a Wallpaper section, off until installed from the Features hub, that lists Apple's stills and the pictures or folders you add and applies one with a click, on the current Space or, with Show on all Spaces, on every Space and display. Thanks to @GabrielDazzi.
- Window Layout can move the pointer to the center of the next display with Move pointer to next display, off by default, with Control-Option-Command-Z as its shortcut. It needs no permission. Thanks to @ancoesat.
- Keep dragged files in the menu bar can place the Shelf at the top center of the screen, as a small badge with the item count, from Position on the Shelf page. Below the menu bar icon stays the default, and the choice waits while Dynamic Island holds the top center. Thanks to @PathGao.
- Command-W closes the focused screenshot preview like Escape, keeping the capture and any saved file, and follows the keyboard layout. Thanks to @samanyudas.
- Dynamic Island can show each new song for a moment, with its cover, title and artist beside the camera, with New track under Indicators in its settings. It is off by default, and a click opens the music section. Thanks to @Yahddyyp.
- Vorssaint is available in Slovak and Ukrainian throughout the app, including the macOS permission prompts, with counts that read right for any number and the settings names each language's macOS uses. Thanks to @rado9904 and @zamai.
- Audio device priority, off until installed from the Features hub, keeps an ordered list of outputs and one of microphones in the menu panel and puts the top connected device in use when a device connects or disconnects, when you reorder a list or when you turn its switch on. A choice made elsewhere stays until the next connection change, and a device the lists have not ranked yet keeps what macOS gives it. Thanks to @zamai and @PathGao.
- The menu bar can count the external USB devices that are connected, and clicking the count lists each device with its maker. Hubs and the billboard devices of USB-C display adapters are left out. Thanks to @kirolos-esmat.
- Window Layout has an Ignore apps list. While a listed app, such as a game, is in front, Window Layout's shortcuts, window dragging and edge snapping pause so the app gets those keys and the mouse, and the menu panel, Command Bar and radial menu can still arrange its windows. Thanks to @delfu.
...
- Dynamic Island shows a download in progress as its arrow and percentage in narrow wings beside the camera, like the timer, instead of a wide strip with a shortened file name. Thanks to @Yahddyyp.
...
- Dynamic Island notices, such as Fully charged, shrink back into the island with their icon and text as they close, instead of vanishing before the island closes. Thanks to Jets for the feedback.
- With Liquid Glass enabled, Dynamic Island's floating quick action buttons take the island's own shade at their height, dark beside its top and as translucent as its lower edge below it, instead of staying solid black. Reduce Transparency and Increase Contrast keep them solid.
...
- Settings lists tools by name and symbol as primary sidebar entries, with direct links to their controls instead of shared category rows. Mixer, Output switcher and Audio device priority each have an entry, and keyboard light shortcuts have their own link and search result under Shortcuts. Thanks to @ilim-cell.
- The Features hub shows Middle click with a trackpad gesture icon instead of a mouse. Thanks to @PathGao.
...
- Dynamic Island's Liquid Glass stays dark while the island opens or closes without its page, and opens up as the page fades in, instead of flashing an empty see-through shell over the windows beneath it or changing at the last moment when the island returns to rest. Thanks to Pinea for the feedback.
...
- An update that asks for your administrator password takes Extra Brightness off the screen first, so the password prompt no longer stays hidden behind it while the menu bar item stops responding. Thanks to @Kernel-Hunter.
- The section search in Dynamic Island uses the room up to the camera, with a shorter placeholder, or none, where the full one would be cut. Thanks to Pinea for the feedback.
- Dynamic Island's clipboard search highlights its top result as you type, the Up and Down arrows move the highlight, and Return pastes or copies it as a click would, instead of the arrows only moving the cursor in the search field. Thanks to Slipshady for the feedback.
...
- With Dismiss the system banner on, a notification in Dynamic Island keeps its Open button after the macOS banner closes when its app is installed but not running, and opens that app, instead of losing the button half a second later. Such notifications also show their app's icon. Thanks to @npcmail010.
- With Show over the menus on, Dynamic Island stays on screen on a display without a camera cutout when the menu bar hides automatically, instead of disappearing when an app opens and coming back when it closes. Thanks to @Yahddyyp.
- With Reduce Motion on, a Dynamic Island page such as the Scratchpad opened from its shortcut fades in once the island has opened, instead of flashing its text in the closed island first. Thanks to @Yahddyyp.
- Dynamic Island keeps the playback controls of players it drives through Automation, such as streaming apps and browsers, as they were when it opens, instead of showing a simpler row for a moment while it checks access to the player again. Thanks to @Yahddyyp.
...
- Uninstall with Homebrew runs only for the app, package and items its confirmation showed, and says the confirmation is no longer valid when another window changed them while it was open. Thanks to @PathGao.
...
- Mic Mute puts back each channel's own level when it unmutes a microphone without a mute switch, instead of giving every channel the same level. Thanks to @PathGao.
...
- The Features hub's energy badge for Radial Menu follows your saved wheels, so a wheel opened with an extra mouse button or a four-finger tap reads Listens to the mouse, and one with only shortcuts reads Nothing at rest.
...
- Clipboard History stays responsive with many screenshots or image files. Thumbnails load in the background in the history window, menu panel, Dynamic Island and Command Bar, so typing a search or previewing many copied images no longer freezes the app.
- Picking an entry in the Clipboard History window beeps when the app it would paste into has quit, and without Accessibility asks for it once and then beeps, instead of doing nothing. The entry is still copied. Thanks to @PathGao.
- Pinned Clipboard History items no longer disappear when the saved history outgrows its size limit. A pin or an edit that would make the pinned items too large to save is refused instead. Thanks to @PathGao.
...
- The Command Bar's brightness command changes only the display under the pointer and beeps when it cannot, instead of changing another display, usually the built-in one. Thanks to @PathGao.
...
- The permission and energy icons beside each Features hub switch explain themselves on hover and in a popover on click, and VoiceOver reads them, instead of showing a tooltip only with the pointer exactly on one small icon. Thanks to @PathGao.
- Exporting settings says when the backup could not be saved, instead of showing nothing as if it had been canceled. Thanks to @PathGao.
- The Super key page says when macOS refuses its keyboard access and how to restore it, instead of showing nothing while the key does nothing. Thanks to @PathGao.
- The Cleaner card says when the last automatic cleanup left items in place, instead of reading like a normal run. Thanks to @PathGao.
- Shelf keeps a drop in order when attachments from other apps arrive with files, links or text, instead of adding the attachments last. Thanks to @PathGao.
- The GPU process list names system processes such as WindowServer on macOS 27, instead of showing only their process number. Thanks to Brain for the feedback.
- With the pointer pushed against the top edge of a display, App Switcher, the Command Bar, Quick Launcher and other panels that follow the pointer open on that display, a full-screen capture takes that display, the quit confirmation and the Shortcut + pointer layout guide appear on it, and a window dropped from Dock Preview lands on its current desktop, instead of on the display with the active window or the one stacked above. Going to a window on another desktop no longer jumps the pointer to the middle of the display it is already on, the capture guide stays on the pointer's display, and the capture loupe's arrow keys keep the pointer on its display at the outer edges.
- Shelf files in Dynamic Island take the first click after the island opens and can be selected, right-clicked and dragged out, and a media drop onto an island that already holds files still offers Files or Optimize media. Thanks to @tenbux.
- Clicking a window to capture it saves the whole window when a sheet or dialog sits on it on macOS 27, instead of part of it next to empty space. Thanks to @ruvelro.
- A brightness key press after a pause keeps the level just set on an external monitor's slider, instead of jumping back to the reading taken before it. Thanks to @PathGao.
- The first CPU reading after a long pause, such as with the panel closed, shows the current load instead of the average of the whole pause, and the CPU alert starts over. Thanks to @PathGao.
- Minimal Dock Preview cards no longer leave empty bands above and below the picture. Thanks to @PathGao.
...
- The Window Layout shortcuts and window dragging groups no longer show an empty row, and the screenshot subfolder and file name fields sit on the right like the other settings.
- Tiling window managers no longer list Vorssaint's floating panels as windows on the current desktop, including the Shelf while you drag selected text, the Command Bar, App Switcher, Clipboard History, capture and recording overlays, pinned screenshots, Dock and Window Layout previews, on-screen indicators and the invisible anchor that places the menu panel when the menu bar is hidden. Thanks to @Yahddyyp.
- The Screen capture page in Settings lists its tools in a menu when the window is too narrow for their buttons, instead of cutting the page off on both sides under the sidebar. Thanks to @Yahddyyp.
