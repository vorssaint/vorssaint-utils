...
- With the pointer pushed against the top edge of a display, a full-screen capture takes that display, the quit confirmation and the Shortcut + pointer layout guide appear on it, and a window dropped from Dock Preview lands on its current desktop, instead of another display such as the one above. The capture loupe's arrow keys also keep the pointer on its display at the edges, instead of jumping to another display.
