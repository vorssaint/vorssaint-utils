...
enum NotchFileToolsTests {
    static func run(_ suite: TestSuite) {
        ...
        for language in AppLanguage.allCases {
            ...
            suite.expect(Mirror(reflecting: strings).children.allSatisfy {
        ...
        suite.expect(NotchDownloadSupport.fraction(completed: 30, total: 100, reportedFraction: 0.3,
                                            indeterminate: false) == 0.3,
               "download percent follows the publisher's real progress")
        for total in [Int64(-1), Int64(0)] {
            suite.expect(NotchDownloadSupport.fraction(completed: 30, total: total, reportedFraction: 0.3,
                                                indeterminate: false) == nil,
                   "unknown totals never become a made-up percentage")
        }
        suite.expect(NotchDownloadSupport.fraction(completed: 30, total: 100, reportedFraction: .nan,
                                            indeterminate: false) == nil,
        ...
        suite.expect(NotchDownloadSupport.expectedURL(for: partial) == destination,
               "only the partial suffix is removed when identifying a finished file")
        suite.expect(NotchDownloadSupport.expectedURL(for: folder.appendingPathComponent("photo.jpg")) == nil,
               "ordinary files are never treated as active downloads")
        suite.expect(!NotchDownloadSupport.isDirectChild(folder.appendingPathComponent("private/deeper/file"), of: folder),
        ...
        suite.expect(NotchDownloadSupport.didFinish(snapshot, at: destination, resourceID: "original"),
               "a renamed partial with the same file identity can complete")
        suite.expect(!NotchDownloadSupport.didFinish(snapshot, at: destination, resourceID: "unrelated")
        ...
        suite.expect(!NotchFileToolsSupport.destinationIsOutsideInputs(destination, inputs: [URL(string: "https://example.com/file")!]),
               "remote input URLs cannot reach the file archive process")
        archiveContainmentContracts(suite)
        collisionContracts(suite)
        publicationContracts(suite)
        renamedPublicationContracts(suite)
    ...
    private static func archiveContainmentContracts(_ suite: TestSuite) {
        ...
        do {
            ...
            suite.expect(!NotchFileToolsSupport.destinationIsOutsideInputs(source.appendingPathComponent("inside.zip"), inputs: [source]),
                   "an archive cannot write its output inside its source directory")
            suite.expect(NotchFileToolsSupport.destinationIsOutsideInputs(source.appendingPathComponent("other.zip"), inputs: [input]),
            ...
            if manager.fileExists(atPath: alias.path) {
                suite.expect(!NotchFileToolsSupport.destinationIsOutsideInputs(alias.appendingPathComponent("inside.zip"), inputs: [source])
            ...
            suite.expect(!NotchFileToolsSupport.destinationIsOutsideInputs(link.appendingPathComponent("inside.zip"), inputs: [source]),
                   "a symbolic link cannot bypass archive containment")
        } catch { suite.expect(false, "archive containment fixtures: \(error)") }
    }

    private static func publicationContracts(_ suite: TestSuite) {
        ...
        do {
            ...
            suite.expect(publication.item(NotchDownloadProgressSnapshot(published), folder: folder)?.fraction == 0.5,
            ...
            suite.expect(missing?.completed == false && missing?.fraction == nil && missing?.active == false,
            ...
            suite.expect(publication.item(NotchDownloadProgressSnapshot(published), folder: folder)?.completed == true,
            ...
            published.completedUnitCount = 10
            suite.expect(publication.item(NotchDownloadProgressSnapshot(published), folder: folder)?.completed == false,
            ...
            suite.expect(publication.item(NotchDownloadProgressSnapshot(published), folder: folder) == nil,
            ...
            suite.expect(moved.item(NotchDownloadProgressSnapshot(renamed), folder: folder)?.completed == false,
            ...
            suite.expect(moved.item(NotchDownloadProgressSnapshot(renamed), folder: folder)?.active == true,
            ...
            renamed.completedUnitCount = 10
            suite.expect(moved.item(NotchDownloadProgressSnapshot(renamed), folder: folder)?.completed == true,
            ...
            renamed.completedUnitCount = 10
            suite.expect(partialPublication.item(NotchDownloadProgressSnapshot(renamed), folder: folder)?.completed == false,
                   "a remaining partial file never becomes a final download just because its counter finishes")
        } catch { suite.expect(false, "published download fixture failed: \(error)") }
    }

    private static func renamedPublicationContracts(_ suite: TestSuite) {
        let manager = FileManager.default
            ...
            for existsInitially in [false, true] {
                for callbackOrder in 0..<3 {
                    ...
                    if callbackOrder == 1 {
                        ...
                        suite.expect(publication.item(NotchDownloadProgressSnapshot(progress), folder: folder)?.completed == false,
                    ...
                    suite.expect(identity == NotchDownloadSupport.fileIdentity(at: final)
                    ...
                    suite.expect(publication.item(NotchDownloadProgressSnapshot(progress), folder: folder)?.completed == true,
    ...
    private static func archiveCancellationContracts(_ suite: TestSuite) {
        let fm = FileManager.default
        ...
        do {
            ...
            suite.expect(try Data(contentsOf: output).prefix(2) == Data([0x50, 0x4B]),
            ...
            suite.expect(child.isRunning, "the cancellation fixture owns a running child")
            operation.cancel(immediately: true)
            let stopped = finished.wait(timeout: .now() + 3) == .success
            suite.expect(stopped && !child.isRunning,
                   "immediate stop terminates an owned child without requiring a future main-loop callback")
            if stopped {
                suite.expect(outcome.cancelled, "a terminated archive is reported as cancelled")
                suite.expect(outcome.staged.map { !fm.fileExists(atPath: $0.deletingLastPathComponent().path) } == true,
                       "archive cancellation removes its private staged directory")
            }
            suite.expect(try Data(contentsOf: output) == original, "cancellation cannot overwrite a previously completed archive")
            ...
            do {
                ...
                suite.expect(false, "a stopped archive cannot restart")
            } catch is CancellationError {
                suite.expect(!madeProcess, "stopping a queued operation prevents even preparing its child")
            }
        } catch { suite.expect(false, "archive cancellation fixture failed: \(error)") }
    }

    private static func durationLoadingContracts(_ suite: TestSuite) {
        ...
        loading.cancel()
        suite.expect(loading.finish(cancelled, duration: 10) == nil,
        ...
        suite.expect(resumed != cancelled && loading.finish(resumed, duration: 120.25) == 120.3,
               "reopening the same input retries a cancelled duration lookup with a new identity")
        loading.cancel()
        suite.expect(loading.start(url: first, tool: .videoCompressor) == nil,
        ...
        suite.expect(loading.finish(replaced, duration: 15) == nil && loading.finish(replacement, duration: 300) == 300,
               "an older input cannot replace the duration of the newly selected video")
        let gif = loading.start(url: second, tool: .gifMaker)!
        suite.expect(loading.finish(gif, duration: nil) == nil,
               "an unavailable duration never marks media defaults as loaded")
        let retry = loading.start(url: second, tool: .gifMaker)!
        suite.expect(loading.finish(retry, duration: .infinity) == nil,
               "a non-finite duration remains retryable")
        suite.expect(loading.start(url: first, tool: .textExtractor) == nil && loading.pending == nil,
        ...
        suite.expect(untrimmed.start == 0 && untrimmed.end == 300,
               "a new input's cleared range exports the full recording while its duration lookup is pending")
    }

    private static func collisionContracts(_ suite: TestSuite) {
        let fm = FileManager.default
        ...
        do {
            ...
            suite.expect(stagedIdentity != nil && stagedIdentity != NotchDownloadSupport.fileIdentity(at: existing),
            ...
            do {
                ...
                suite.expect(false, "an output collision must fail")
            } catch {
                suite.expect(try Data(contentsOf: existing) == Data("original".utf8),
                       "archive and shelf media outputs preserve a colliding original")
                suite.expect(fm.fileExists(atPath: staged.path), "a rejected install leaves staged data available for cleanup")
            }
            ...
            suite.expect(try Data(contentsOf: newOutput) == Data("generated".utf8),
                   "a unique shelf output is installed intact")
            suite.expect(!fm.fileExists(atPath: staged.path), "installing output does not retain a redundant staged copy")
            suite.expect(NotchDownloadSupport.fileIdentity(at: newOutput) == stagedIdentity,
                   "a real filesystem rename preserves the identity used by download completion")
        } catch { suite.expect(false, "file output contract fixture failed: \(error)") }
    }
}
