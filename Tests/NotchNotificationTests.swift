...
enum NotchNotificationTests {
    static func run(_ suite: TestSuite) {
        ...
        suite.expect(NotchEvent.systemNotification.duration == 3 && NotchEvent.timer.duration == 6,
        ...
        suite.expect(content.app == "Chat" && content.title == "Alex" && content.body == "Hello",
               "notifications keep application, conversation and message separate")
        suite.expect(NotchNotificationSupport.content(from: texts + [Text(identifier: "title", value: "Sam")]) == nil,
               "stacked senders are never combined into one notification")
        suite.expect(NotchNotificationSupport.content(from: texts + [Text(identifier: "body", value: "Second")]) == nil,
               "multiple messages require separate notification identities")
        suite.expect(NotchNotificationSupport.content(from: [Text(identifier: "body", value: "No sender")]) == nil,
               "a body without a title cannot become a notification")
        suite.expect(NotchNotificationSupport.content(from: [Text(identifier: "title", value: String(repeating: "a", count: 16_385))]) == nil,
               "notification text has a bounded memory budget")
        suite.expect(NotchNotificationSupport.content(from: [Text(identifier: "", value: "Alex"), Text(identifier: "", value: "Hello")])?.body == "Hello",
               "legacy banners with two static labels preserve sender and message")
        suite.expect(NotchNotificationSupport.content(from: texts + [Text(identifier: "editableText", value: "Private draft")]) == content,
               "editable native content never becomes part of a mirrored notification")
        suite.expect(NotchNotificationSupport.nativeIdentity("AXNotificationListItems") == nil
        ...
        suite.expect(NotchNotificationSupport.applicationLabel(from: "Chat, Inc., Alex, Sam, Group, Hello, everyone", content: attributedContent) == "Chat, Inc.",
               "formatted descriptions preserve commas in app names, senders and messages")
        suite.expect(NotchNotificationSupport.applicationLabel(from: "Chat, Different message", content: attributedContent) == nil
        ...
        suite.expect(NotchNotificationSupport.sourceBundleIdentifier(for: [" chat "], applications: applications) == "test.chat",
               "a complete source label selects its application icon without depending on label capitalization")
        suite.expect(NotchNotificationSupport.sourceBundleIdentifier(for: ["\u{200e}Chat"], applications: applications) == "test.chat"
        ...
        suite.expect(NotchNotificationSupport.sourceBundleIdentifier(for: [""], applications: applications) == nil
        ...
        suite.expect(shortBanner.compactTitle == "Alex" && shortBanner.compactDetail == "Test",
               "compact notifications keep the sender and message in their separate wings")
        suite.expect(longBanner.compactDetail == longBanner.body,
               "visual truncation never discards the full stored notification text")
        suite.expect(NotchNotificationContent(app: "Chat", title: "", subtitle: "Group", body: "Test").compactTitle == "Chat",
        ...
        suite.expect(conversation.compactDetail == "Trip · Hello",
               "conversation context and the message share the compact detail without extra rows")
        suite.expect(conversation.accessibilityText == "Chat, Alex, Trip, Hello",
               "a banner announces source, sender, conversation and message in reading order")
        suite.expect(NotchNotificationContent(app: "", title: "Alex", subtitle: "", body: "Hello").accessibilityText == "Alex, Hello",
               "unavailable source metadata never duplicates or corrupts the announcement")
        let requestIdentity = "request." + UUID().uuidString
        suite.expect(NotchNotificationSupport.nativeIdentity(requestIdentity) == requestIdentity,
        ...
        suite.expect(inbox.update([existing]).isEmpty && inbox.items.isEmpty,
               "enabling mirroring does not replay banners already on screen")
        suite.expect(inbox.update([existing, first, first]).map(\.id) == [first.id] && inbox.items.count == 1,
               "repeated layout callbacks deliver each new notification once")
        suite.expect(inbox.update([first, second]).map(\.id) == [second.id] && inbox.items.count == 2,
               "identical text from different notifications remains distinct")
        inbox.dismiss(second.id)
        suite.expect(inbox.update([first, second]).isEmpty && !inbox.items.contains(where: { $0.id == second.id }),
               "a dismissed mirror does not return when the native banner changes layout")
        _ = inbox.update([])
        suite.expect(inbox.items.first?.canOpen == false,
               "expired native banners lose their actions while their mirrored text remains")
        for _ in 0..<300 { _ = inbox.update([item()]) }
        suite.expect(inbox.items.count == NotchNotificationSupport.maximumItems, "the session inbox stays bounded during bursts")
        ...
        suite.expect(!NotchSupport.routes(.systemNotification, in: defaults) && !NotchNotificationSupport.dismissesNative(in: defaults), "the hub gates the reader, banners and native closing together")
        ...
        suite.expect(!NotchNotificationSupport.isEnabled(in: defaults), "hiding notifications also stops reading messages")
        ...
        suite.expect(AppFeature.notchNotifications.permissions == [.accessibility],
