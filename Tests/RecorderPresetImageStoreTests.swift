...
enum RecorderPresetImageStoreTests {
    static func run(_ suite: TestSuite) {
        ...
        do {
            ...
            else { suite.expect(false, "preset image fixture can be created"); return }
            CGImageDestinationAddImage(destination, image, nil)
            guard CGImageDestinationFinalize(destination) else {
                suite.expect(false, "preset image fixture can be written")
                return
            }
            ...
            guard let captured = store.capture([overlay]), captured.count == 1 else {
                suite.expect(false, "preset captures its image independently")
                return
            }
            let storedURL = URL(fileURLWithPath: captured[0].path)
            suite.expect(storedURL != originalURL && (try? Data(contentsOf: storedURL)) == bytes,
                   "saving a preset creates an independent image with identical pixels")
            suite.expect(storedURL.lastPathComponent == originalURL.lastPathComponent,
                   "preset copies preserve the picked image's filename")
            let attributes = try manager.attributesOfItem(atPath: storedURL.path)
            suite.expect((attributes[.posixPermissions] as? NSNumber)?.intValue == 0o600,
            ...
            guard let restored = reopened.restore(decoded.images ?? [], into: targetTake, duration: 37),
                ...
                suite.expect(false, "a reloaded preset restores its image after the original recording is gone")
                return
            }
            suite.expect(restored[0].start == 0 && restored[0].end == 37,
                   "preset pictures span the whole destination video, not the original interval")
            suite.expect(restored[0].anchor == .topTrailing && restored[0].size == 0.31
                    && restored[0].opacity == 0.63,
                   "preset image position, size and transparency survive save and reload")
            suite.expect(restored[0].path != captured[0].path && restored[0].id != captured[0].id
            ...
            suite.expect(shorter?.first?.start == 0 && shorter?.first?.end == 0.8,
            ...
            suite.expect(applied.images == restored && applied.trimStart == 1 && applied.trimEnd == 8
            ...
            suite.expect(prepared.applying(to: applied).images.count == 1,
            ...
            suite.expect(oldPreset.images == nil && oldPreset.applying(to: applied).images == restored,
                   "presets saved before picture support still load and preserve existing pictures")
            suite.expect(RecorderEditPreset(name: "No picture", document: RecorderEditDocument())
            ...
            suite.expect(portable?.first?.id == saved.id && portable?.first?.images == nil,
            ...
            if let portableData {
                ...
                suite.expect(try JSONDecoder().decode([RecorderEditPreset].self, from: carried).first?.images == captured,
                       "restoring settings on this Mac keeps matching presets' private images")
                suite.expect(SettingsBackupSupport.preservingLocalPresetImages(restored: portableData, local: nil)
                        == portableData,
            ...
            suite.expect(reopened.restore([captured[0], missing], into: targetTake, duration: 5) == nil
            ...
            suite.expect(reopened.restore([overlay], into: targetTake, duration: 5) == nil,
            ...
            suite.expect(reopened.restore([escaped], into: targetTake, duration: 5) == nil,
                   "a linked preset folder never imports images from outside the store")
            reopened.remove([escaped])
            suite.expect(manager.fileExists(atPath: outside.appendingPathComponent("Logo.png").path),
                   "preset cleanup never follows a linked folder into somebody else's files")

            reopened.remove(captured)
            suite.expect(!manager.fileExists(atPath: storedURL.path)
                    && (try? Data(contentsOf: URL(fileURLWithPath: restored[0].path))) == bytes,
            ...
            suite.expect(reopened.restore(captured, into: targetTake, duration: 5) == nil
                    && !manager.fileExists(atPath: targetTake.folder.path),
                   "an apply finishing after the editor closes never recreates its recording")
        } catch {
            suite.expect(false, "preset image round-trip failed: \(error)")
        }
    }
}
