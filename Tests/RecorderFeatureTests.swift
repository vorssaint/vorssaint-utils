...
enum RecorderFeatureTests {
    static func run(_ suite: TestSuite) {
        ...
        suite.expect(Defaults.registeredDefaults[DefaultsKey.recorderShortcutEnabled] as? Bool == false,
               "the screen recorder shortcut ships off like every new feature")
        suite.expect(Defaults.registeredDefaults[DefaultsKey.recorderShortcut] as? String
                == "control+option+command:23",
               "the default recording shortcut sits next to the screenshot's own")
        suite.expect(Defaults.registeredDefaults[DefaultsKey.panelUtilityScreenRecorder] as? Bool == true,
        ...
        suite.expect(firstWindowNeedsPromotion && !secondWindowNeedsPromotion
        ...
        suite.expect(ScreenshotSupport.countdownRingProgress(elapsed: 0) == 1
        ...
        suite.expect(ScreenshotSupport.countdownRingProgress(elapsed: -1) == 1
        ...
        suite.expect(FeatureVisibilitySupport.features(for: .screenshot)
        ...
        suite.expect([AppFeature.screenshot, .screenRecorder, .screenOCR, .colorPicker]
                .allSatisfy { $0.settingsDestination.page == .screenshot },
               "every screen tool opens its mode inside the centralized Settings page")
        suite.expect(SettingsBackupSupport.exportKeys().contains(DefaultsKey.recorderShortcutEnabled)
        ...
        suite.expect(RecordingShareDuration.allCases.map(\.rawValue) == [3_600, 21_600],
        ...
        suite.expect(RecordingSharingSupport.uploadURL(endpoint: recordingShareEndpoint,
        ...
        suite.expect(RecordingSharingSupport.record(response: recordingResponse,
        ...
        let recordingPlan = RecordingSharingSupport.encodingPlan(
            ...
            sourceFrameRate: 60,
            hasAudio: true)
        suite.expect(recordingPlan != nil
        ...
        if let recordingPlan {
            let plannedBytes = (recordingPlan.videoBitRate + recordingPlan.audioBitRate)
                * 30 / 8
            suite.expect(plannedBytes < RecordingSharingSupport.targetUploadBytes,
                   "the first compression pass stays inside the upload budget")
        }
        suite.expect(RecordingSharingSupport.encodingPlan(
            duration: 3_600,
            baseSize: CGSize(width: 1920, height: 1080),
            sourceFrameRate: 30,
            hasAudio: true) == nil,
               "sharing refuses a duration that cannot fit without unusable quality")
        suite.expect(RecordingSharingSupport.retryScale(current: 1,
        ...
        suite.expect(displayPixels.contains(overflowing),
               "an area that runs off the display is brought back inside it")
        suite.expect(overflowing.width >= RecorderSupport.minimumSide
        ...
        suite.expect(RecorderSupport.elapsedLabel(seconds: -5) == "0:00",
               "a clock that never went forward still reads as zero")

        suite.expect(RecorderSupport.trustsSystemAudioTap(previously: false, tapHeardSound: true,
        ...
        suite.expect(!RecorderSupport.trustsSystemAudioTap(previously: true, tapHeardSound: false,
                                                     streamHeardSound: true),
        ...
        suite.expect(pauseTimeline.elapsed(since: 0, at: 7) == 3
        ...
        suite.expect(pauseTimeline.resume(at: 8) && !pauseTimeline.resume(at: 9),
               "a recording resumes one open pause only once")
        suite.expect(pauseTimeline.sampleTime(start: 2, duration: 0.01, since: 0) == 2
        ...
        suite.expect(RecorderSupport.sanitizedQuality("high") == .high
        ...
        let smallSize = RecorderSupport.outputSize(source: CGSize(width: 2940, height: 1912),
                                                   quality: .small)
        suite.expect(smallSize == CGSize(width: 1470, height: 956),
               "the small preset halves the picture and stays on even pixels")
        suite.expect(RecorderSupport.outputSize(source: CGSize(width: 2940, height: 1912), quality: .high)
        ...
        let smallRate = RecorderSupport.averageBitRate(width: 1470, height: 956, fps: 60,
                                                       quality: .small)
        suite.expect(highRate > balancedRate && balancedRate > smallRate,
               "the presets differ by bits, and only the small one by pixels")
        suite.expect(RecorderSupport.averageBitRate(width: 64, height: 64, fps: 30, quality: .small)
                >= 800_000,
               "even a tiny area gets a usable stream")
        suite.expect(highRate <= 60_000_000,
        ...
        suite.expect(swept == [leftBehind, deadOnArrival],
               "the sweep only takes what a crash or a quit left behind")
        suite.expect(!swept.contains(beingEdited),
               "a recording whose editor is open is never swept out from under it")
        suite.expect(!swept.contains(beingWritten),
        ...
        do {
            ...
            suite.expect((try? Data(contentsOf: savedDestination)) == masterBytes,
                   "direct save copies the complete recording to its destination")
            suite.expect(!FileManager.default.fileExists(atPath: savedFolder.path),
                   "direct save removes its internal take after the copy succeeds")
        } catch {
            suite.expect(false, "a valid direct save succeeds")
        }
        ...
        suite.expect(directSaveFailed,
               "direct save reports a destination that cannot be written")
        suite.expect(FileManager.default.fileExists(atPath: recoverableTake.videoURL.path),
        ...
        suite.expect(RecorderTakeStore.shared.importVideo(at: importSource, into: importedTake),
               "an ordinary video is staged as an editor take")
        suite.expect((try? Data(contentsOf: importedTake.videoURL)) == masterBytes,
        ...
        suite.expect((try? Data(contentsOf: importSource)) == changedSourceBytes,
        ...
        suite.expect(abandonedStaging.deletingLastPathComponent().path == directSaveRoot.path
        ...
        suite.expect((try? Data(contentsOf: exportDestination)) == alreadySavedBytes,
        ...
        suite.expect(RecorderSupport.commitExport(from: finishedStaging, to: exportDestination)
        ...
        suite.expect(RecorderSupport.commitExport(from: freshStaging, to: freshDestination)
                && (try? Data(contentsOf: freshDestination)) == exportedBytes,
               "an export to a name nothing uses yet lands under that name")
        suite.expect(!RecorderSupport.commitExport(
        ...
        suite.expect(RecorderSupport.pendingStartIsAuthorized(requestGeneration: 4,
                                                        currentGeneration: 4,
        ...
        suite.expect(recorderStartGate.begin() && recorderStartGate.isAuthorized,
               "recorder start gate admits one authorized capture start")
        suite.expect(recorderStartGate.cancelAndClaimStop()
        ...
        suite.expect(recorderStartFinished.wait(timeout: .now() + 0.05) == .timedOut,
               "recorder stop waits while capture start is suspended")
        recorderStartGate.finish()
        suite.expect(recorderStartFinished.wait(timeout: .now() + 0.5) == .success
        ...
        suite.expect(recorderFailureGate.begin() && recorderFailureGate.claimStartFailure()
        ...
        suite.expect(!captureLifecycle.acceptsSamples && captureLifecycle.beginStart()
        ...
        suite.expect(!captureLifecycle.acceptsSamples && !captureLifecycle.isRunning
        ...
        suite.expect(RecorderSupport.shouldStopForDisk(freeBytes: 100_000_000)
                && !RecorderSupport.shouldStopForDisk(freeBytes: 10_000_000_000),
               "a recording already running stops before it fills the disk")
        suite.expect(RecorderSupport.minimumFreeBytesToContinue < RecorderSupport.minimumFreeBytesToStart,
               "stopping a recording is allowed to get closer to full than starting one")
        suite.expect(!RecorderTakeStore.canImport(fileSize: 600_000_000,
        ...
        suite.expect(wholeClip.start == 0 && wholeClip.end == 12,
        ...
        suite.expect(inside.start == 2 && inside.end == 9 && inside.duration == 7,
        ...
        suite.expect(collapsed.duration >= RecorderSupport.minimumTrimSeconds - 1e-9,
        ...
        suite.expect(past.end <= 12 && past.start >= 0 && past.duration > 0,
               "a trim past the end of the recording is brought back inside it")
        suite.expect(RecorderSupport.sanitizedTrim(start: 0, end: 5, duration: 0).duration == 0,
               "a recording with no duration produces no clip instead of a broken one")

        suite.expect(RecorderSupport.filmstripTimes(duration: 10, count: 4)
                == [1.25, 3.75, 6.25, 8.75],
               "filmstrip frames are sampled at the middle of each slot")
        suite.expect(RecorderSupport.filmstripTimes(duration: 0, count: 4).isEmpty
                && RecorderSupport.filmstripTimes(duration: 10, count: 0).isEmpty,
               "an empty recording asks for no thumbnails")

        suite.expect(RecorderSupport.gifFrameCount(duration: 10, fps: 12) == 120,
               "a GIF holds one frame per tick of its own rate")
        suite.expect(RecorderSupport.gifFitsBudget(duration: 10, fps: 12)
        ...
        suite.expect(Int(RecorderSupport.maximumGIFSeconds(fps: 12)) == 25,
               "the refusal can say exactly how long a GIF may be at that rate")
        suite.expect(RecorderSupport.gifDelay(fps: 10) == 0.1,
        ...
        suite.expect(documentRoundTrip.trimStart == 1 && documentRoundTrip.trimEnd == 4
        ...
        suite.expect(RecorderEditDocument.decoded(Data("not json".utf8)).trimStart == 0,
        ...
        suite.expect(recoveredZooms.zoomSegments.count == 1
        ...
        let disabledZooms = RecorderEditDocument(zoomEnabled: false)
            .restoringAutomaticZooms(clicks: [click], duration: 10)
        suite.expect(disabledZooms.zoomSegments.isEmpty,
        ...
        let preservedZooms = RecorderEditDocument(zoomEnabled: true,
            ...
            .restoringAutomaticZooms(clicks: [click], duration: 10)
        suite.expect(preservedZooms.zoomSegments == [intentionalZoom],
        ...
        suite.expect(RecorderTypingTrack.decoded(typingTrack.encoded()) == typingTrack,
        ...
        suite.expect(appliedPreset.backdrop == "style" && appliedPreset.zoomAmount == 2.4
        ...
        suite.expect(abs(flatAlpha - 0.18899) < 0.0001,
               "the one-pole coefficient matches the closed form at 60 fps")
        suite.expect(RecorderMotion.onePoleAlpha(cutoff: 0, dt: 1.0 / 60) == 1,
        ...
        suite.expect(filtered.count == step.count,
               "smoothing returns as many samples as it was given")
        suite.expect(abs(filtered[59] - 0.5) < 0.2,
               "the smoothed step crosses halfway AT the step, not after it")
        suite.expect(filtered[0] < 0.02 && filtered[119] > 0.98,
        ...
        suite.expect(monotonic, "smoothing a step never overshoots or rings")

        let clicks = [RecorderMotion.Click(time: 1.0, isDown: true),
                      RecorderMotion.Click(time: 1.3, isDown: false)]
        suite.expect(RecorderMotion.anchorWeight(at: 1.0, clicks: clicks) == 1,
               "at the moment of the press the pointer is exactly where it really was")
        suite.expect(RecorderMotion.anchorWeight(at: 1.15, clicks: clicks) == 1,
               "through a drag the pointer tracks the truth, or what is dragged comes loose")
        suite.expect(RecorderMotion.anchorWeight(at: 0.5, clicks: clicks) == 0,
               "far from a click the smoothed path is used as it is")
        suite.expect(RecorderMotion.anchorWeight(at: 0.94, clicks: clicks) > 0
        ...
        suite.expect(RecorderMotion.anchorWeight(at: 5.0, clicks: openDrag) == 1,
               "a recording that ended mid drag keeps tracking to the end")

        suite.expect(RecorderMotion.smoothstep(0) == 0 && RecorderMotion.smoothstep(1) == 1
                && RecorderMotion.smoothstep(0.5) == 0.5,
               "the easing runs from nothing to everything through the middle")
        suite.expect(RecorderMotion.smootherstep(-4) == 0 && RecorderMotion.smootherstep(9) == 1,
        ...
        suite.expect(oneZoom.count == 1,
               "a burst of clicks is one zoom that holds, not a flicker")
        suite.expect(oneZoom.first.map { $0.start < 2.0 } == true,
        ...
        let typingZoom = RecorderMotion.zoomSegments(
            ...
            typingTimes: [4, 5, 6],
            duration: 20)
        suite.expect(typingZoom.first.map { $0.end >= 7.39 } == true,
               "typing after a click keeps that zoom on the field until writing stops")
        let delayedTypingZoom = RecorderMotion.zoomSegments(
            clicks: [RecorderMotion.Click(time: 2, isDown: true)],
            typingTimes: [9, 10],
            duration: 20)
        suite.expect(delayedTypingZoom.first.map { $0.end >= 11.39 } == true,
               "a pause after focusing a field does not lose its typing zoom")
        suite.expect(RecorderMotion.zoomSegments(clicks: [], typingTimes: [2, 3], duration: 20).isEmpty,
        ...
        suite.expect(RecorderMotion.zoomSegments(clicks: farApart, duration: 20).count == 2,
               "clicks far apart get a zoom each")
        suite.expect(RecorderMotion.zoomSegments(
        ...
        suite.expect(RecorderMotion.zoomSegments(clicks: burst, duration: 20)
                .allSatisfy { $0.end <= 20 - RecorderMotion.zoomEndMargin + 0.001 },
               "no clip ends in the middle of a zoom")
        suite.expect(RecorderMotion.zoomSegments(clicks: [], duration: 20).isEmpty,
               "a recording with no clicks has no zoom")

        let segment = RecorderMotion.ZoomSegment(start: 2, end: 6)
        suite.expect(RecorderMotion.zoomProgress(at: 1.5, segments: [segment]) == 0,
               "before the segment the picture is untouched")
        suite.expect(RecorderMotion.zoomProgress(at: 4, segments: [segment]) == 1,
               "in the middle of the segment the zoom is fully in")
        let rampingIn = RecorderMotion.zoomProgress(at: 2.2, segments: [segment])
        suite.expect(rampingIn > 0 && rampingIn < 1, "the zoom eases in rather than cutting")
        let rampingOut = RecorderMotion.zoomProgress(at: 6.3, segments: [segment])
        suite.expect(rampingOut > 0 && rampingOut < 1, "the zoom eases out rather than cutting")
        ...
        suite.expect(abs(spring.value - 1) < 0.02,
        ...
        suite.expect(springTrace.first == 0 && springTrace.last ?? 0 > 0.9,
               "a springed timeline starts where it started and settles on the target")
        suite.expect(springTrace.max() ?? 0 <= 1.2,
        ...
        let cluster = RecorderMotion.focusClusters(
            ...
            frameRate: 60, zoom: 2)
        suite.expect(cluster.count >= 2,
               "a jump across the screen starts a new place to look at")
        suite.expect(RecorderMotion.focus(at: 0, clusters: cluster) == cluster[0].center,
               "at the start the picture looks where the pointer started")

        suite.expect(RecorderMotion.pressScale(at: 1.0, clicks: clicks) == RecorderMotion.pressPunchScale,
               "the pointer is at its smallest exactly while the button is down")
        suite.expect(RecorderMotion.pressScale(at: 0.5, clicks: clicks) == 1,
               "away from a click the pointer is its normal size")
        suite.expect(RecorderMotion.ringProgress(at: 1.0, clicks: clicks) == 0,
               "the ring is born at the click")
        suite.expect(RecorderMotion.ringProgress(at: 2.0, clicks: clicks) == nil,
        ...
        suite.expect(scanFrames > 5_000 && scanClicks.count > 500,
               "the cursor comparison covers a long recording and a busy click list")
        suite.expect(cursorMatches,
        ...
        suite.expect(uniform.count == 10, "the track is resampled onto the output grid")
        suite.expect(abs(uniform[5].x - 0.5) < 0.01,
               "positions between two samples are interpolated in time")
        suite.expect(RecorderMotion.resampled([], frameRate: 10, duration: 1).count == 10,
        ...
        suite.expect(kept.count == 2 && kept[0] == 0...5 && kept[1] == 8...20,
               "a cut in the middle leaves the two pieces around it")
        suite.expect(abs(RecorderTimeline.outputDuration(trim: cutTrim, cuts: oneCut) - 17) < 0.001,
               "the finished video is shorter by exactly what was cut")
        suite.expect(abs(RecorderTimeline.sourceTime(forOutput: 4, trim: cutTrim, cuts: oneCut) - 4) < 0.001,
               "before the cut the two clocks agree")
        suite.expect(abs(RecorderTimeline.sourceTime(forOutput: 6, trim: cutTrim, cuts: oneCut) - 9) < 0.001,
               "after the cut the recording is ahead of the video by the cut's length")
        suite.expect(RecorderTimeline.outputTime(forSource: 6.5, trim: cutTrim, cuts: oneCut) == nil,
               "a moment that was cut out does not exist in the finished video")
        suite.expect(abs((RecorderTimeline.outputTime(forSource: 12, trim: cutTrim, cuts: oneCut) ?? 0)
                    - 9) < 0.001,
        ...
        for step in stride(from: 0.0, through: 17.0, by: 0.37) {
            ...
            suite.expect(abs((back ?? -99) - step) < 0.01,
                   "the recording clock and the video clock are inverses of each other")
        }
        suite.expect(RecorderTimeline.normalized(cuts: [RecorderTimeline.Cut(start: 2, end: 6),
                                                  RecorderTimeline.Cut(start: 5, end: 9)],
                                           duration: 20)
                == [RecorderTimeline.Cut(start: 2, end: 9)],
               "two cuts that touch become one")
        suite.expect(RecorderTimeline.normalized(cuts: [RecorderTimeline.Cut(start: 3, end: 3.02)],
                                           duration: 20).isEmpty,
        ...
        suite.expect(tidy.count == 2 && tidy[0].end <= tidy[1].start + 0.001,
        ...
        suite.expect(moved.start >= tidy[0].end - 0.001,
               "dragging a zoom stops at its neighbour instead of pushing it")
        suite.expect(abs(moved.duration - tidy[1].duration) < 0.001,
        ...
        suite.expect(squeezed.duration >= RecorderTimeline.minimumSegment - 1e-9,
               "a zoom cannot be dragged shorter than something you could grab again")
        suite.expect(RecorderTimeline.slotForNewSegment(at: 10, existing: tidy, duration: 20) != nil,
               "there is room for a new zoom in an empty stretch")
        suite.expect(RecorderTimeline.slotForNewSegment(at: 2, existing: tidy, duration: 20)
        ...
        suite.expect(state.progress == 1 && state.amount == 2 && state.focus != nil,
        ...
        suite.expect(abs(RecorderMotion.travelParameter(exactFocus: 0.9, zoom: 2) - 1) < 0.001,
               "a spot picked near the edge pins the view to that edge")
        suite.expect(abs(RecorderMotion.travelParameter(exactFocus: 0.5, zoom: 2) - 0.5) < 0.001,
        ...
        suite.expect(opacity.count == parked.count, "every frame gets an opacity")
        suite.expect(opacity[10] > 0.95, "a pointer that just stopped is still solid")
        suite.expect(opacity[60 * 5] < 0.05, "a pointer parked for seconds is out of the way")
        suite.expect(opacity[60 * 6] > 0.95, "it is fully back the instant it moves, not after")
        suite.expect(opacity.allSatisfy { $0 >= 0 && $0 <= 1 }, "opacity never leaves its range")
        let alwaysMoving = (0..<120).map { CGPoint(x: Double($0) * 0.01, y: 0.5) }
        suite.expect(RecorderMotion.pointerOpacity(alwaysMoving, frameRate: 60).allSatisfy { $0 > 0.95 },
        ...
        suite.expect(caption.opacity(at: 1.9) == 0 && caption.opacity(at: 6.1) == 0,
               "text is not there before it starts or after it ends")
        suite.expect(caption.opacity(at: 4) == 1, "text is solid in the middle of its own time")
        suite.expect(caption.opacity(at: 2.1) > 0 && caption.opacity(at: 2.1) < 1,
               "text eases in instead of appearing on one frame")
        suite.expect(caption.opacity(at: 5.9) > 0 && caption.opacity(at: 5.9) < 1,
               "text eases out the same way")
        let blink = RecorderTextOverlay(text: "Hi", start: 1, end: 1.3)
        suite.expect(blink.opacity(at: 1.15) > 0,
               "a very short caption still gets to be visible at its middle")
        suite.expect(RecorderTextOverlay(text: "x", start: 5, end: 5.05)
                .sanitized(duration: 10) == nil,
               "a caption too short to read is not kept")
        suite.expect(RecorderTextOverlay(text: "x", start: 9, end: 30)
                .sanitized(duration: 10)?.end == 10,
               "a caption that runs past the recording is brought back inside it")
        suite.expect(RecorderTextOverlay(text: "x", start: 1, end: 3, size: 9)
        ...
        suite.expect(RecorderTextOverlay.Anchor.bottom.unitPoint.y == 1
        ...
        suite.expect(mark.opacity(at: 1.9) == 0 && mark.opacity(at: 6.1) == 0,
               "a picture is not there before it starts or after it ends")
        suite.expect(mark.opacity(at: 4) == 1,
               "a picture is at full strength in the middle of its own time")
        suite.expect(mark.opacity(at: 2.1) > 0 && mark.opacity(at: 2.1) < 1,
               "a picture eases in like a caption instead of appearing on one frame")
        suite.expectClose(RecorderImageOverlay(path: "/tmp/logo.png", start: 0, end: 10, opacity: 0.5)
                        .opacity(at: 5),
                    0.5,
                    "a picture never goes past the strength it was given")
        suite.expect(RecorderImageOverlay(path: "   ", start: 1, end: 5).sanitized(duration: 10) == nil,
               "a picture with no file behind it is dropped rather than drawn")
        suite.expect(RecorderImageOverlay(path: "/tmp/logo.png", start: 9, end: 30)
                .sanitized(duration: 10)?.end == 10,
               "a picture that runs past the recording is brought back inside it")
        suite.expect(RecorderImageOverlay(path: "/tmp/logo.png", start: 5, end: 5.05)
                .sanitized(duration: 10) == nil,
               "a picture too brief to see is not kept")
        suite.expect(RecorderImageOverlay(path: "/tmp/logo.png", start: 1, end: 5, size: 9)
        ...
        suite.expect(RecorderImageOverlay(path: "/tmp/logo.png", start: 1, end: 5, opacity: 0)
        ...
        suite.expect(RecorderImageOverlay.drawnSize(source: CGSize(width: 200, height: 100),
                                              size: 0.5,
        ...
        suite.expect(RecorderImageOverlay.drawnSize(source: CGSize(width: 100, height: 1000),
                                              size: 0.6,
                                              canvas: CGSize(width: 1000, height: 500))
        ...
        for canvas in [CGSize(width: 1000, height: 500), CGSize(width: 500, height: 1000)] {
            for source in [CGSize(width: 100, height: 1000), CGSize(width: 1000, height: 100)] {
                for anchor in RecorderImageOverlay.Anchor.allCases {
                    ...
                    suite.expect(drawn.width > 0 && drawn.height > 0
        ...
        suite.expect(RecorderTextOverlay.Anchor.bottomTrailing
                .origin(of: CGSize(width: 100, height: 50),
        ...
        suite.expect(markedDocument.images == [mark],
               "a picture written next to the recording comes back exactly as it was")
        suite.expect(RecorderEditDocument().affectsPicture(markedDocument)
        ...
        suite.expect(RecorderEditDocument(images: [RecorderImageOverlay(path: "", start: 1, end: 5)])
        ...
        if let picture = pictureContext?.makeImage(),
           let destination = CGImageDestinationCreateWithURL(pictureSource as CFURL,
            ...
            suite.expect(CGImageDestinationFinalize(destination), "the image import fixture is written")
        } else {
            suite.expect(false, "the image import fixture can be created")
        }
        ...
        suite.expect(storedPicture != nil && secondPicture != nil && storedPicture != secondPicture,
               "images with the same filename get independent copies inside the recording")
        if let storedPicture {
            suite.expect(MediaSupport.imageThumbnail(at: storedPicture, maxPixel: 2) != nil,
            ...
            suite.expect(permissions?.intValue == 0o600,
                   "the private picture copy is readable only by its owner")
            try? Data("replacement".utf8).write(to: pictureSource)
            suite.expect((try? Data(contentsOf: storedPicture)) == originalPictureBytes,
        ...
        suite.expect(RecorderTakeStore.shared.importImage(at: badPicture, into: pictureTake) == nil,
               "an unreadable picture never becomes an invisible timeline block")
        suite.expect((try? FileManager.default.contentsOfDirectory(atPath: pictureTake.folder.path))?
        ...
        suite.expect(FileManager.default.fileExists(atPath: badPicture.path),
        ...
        suite.expect(blur.covers(2) && blur.covers(4) && blur.covers(6)
                && !blur.covers(1.99) && !blur.covers(6.01),
               "a blur hides its area on every frame of its block and never eases in or out")
        suite.expect(blur.pixelRect(in: CGSize(width: 1000, height: 500))
        ...
        suite.expect(RecorderBlurRegion(start: 1, end: 1.1).sanitized(duration: 10) == nil,
               "a blur too short to matter is not kept")
        suite.expect(RecorderBlurRegion(start: 8, end: 30).sanitized(duration: 10)?.end == 10,
        ...
        suite.expect(spilled.map { abs($0.x) < 1e-9 && abs($0.y - 0.9) < 1e-9
        ...
        suite.expect(RecorderBlurRegion(start: 1, end: 3,
        ...
        suite.expect(backwards.map { abs($0.minX - 0.2) < 1e-9 && abs($0.minY - 0.3) < 1e-9
                && abs($0.width - 0.6) < 1e-9 && abs($0.height - 0.4) < 1e-9 } == true,
               "a drag in any direction produces the same area")
        suite.expect(RecorderBlurRegion.normalizedRect(from: CGPoint(x: 0.5, y: 0.5),
                                                 to: CGPoint(x: 0.502, y: 0.9)) == nil,
               "a drag too thin to be seen leaves the blur where it was")
        suite.expect(RecorderBlurRegion(start: 0, end: 5).rect == RecorderBlurRegion.defaultRect,
               "a new blur lands in the middle until its area is drawn")
        suite.expect(RecorderSupport.blurBlockSize(for: CGSize(width: 300, height: 24)) == 8
        ...
        suite.expect(RecorderEditDocument().affectsPicture(blurredDocument)
        ...
        suite.expect(RecorderEditDocument(blurs: [RecorderBlurRegion(start: 4, end: 4.05)])
        ...
        suite.expect(stagePoint.map { abs($0.x - 0.6) < 1e-9 && abs($0.y - 0.5) < 1e-9 } == true,
               "a point on the stage maps through the letterbox into the picture's own space")
        suite.expect(RecorderSupport.unitPoint(at: CGPoint(x: 10, y: 10),
                                         in: CGSize(width: 500, height: 500),
        ...
        suite.expect(RecorderSupport.unitPoint(at: .zero, in: .zero, sourceSize: .zero) == nil,
        ...
        suite.expect(trackRoundTrip.samples.count == 1 && trackRoundTrip.clicks.count == 1
                && trackRoundTrip.systemScale == 2,
               "a pointer track survives being written and read back")
        suite.expect(abs(trackRoundTrip.samples[0].point.x - 0.25) < 1e-6
        ...
        suite.expect(!RecorderPointerTrack.decoded(Data(truncated)).samples.isEmpty,
               "a track cut short by a crash still gives back everything it did write")
        suite.expect(RecorderPointerTrack.decoded(Data("nonsense".utf8)).isEmpty,
               "an unrelated file is not mistaken for a pointer track")
        suite.expect(RecorderPointerTrack.decoded(nil).isEmpty,
        ...
        suite.expect(RecorderPointerTrack.decoded(shapedTrack.encoded()) == shapedTrack,
        ...
        suite.expect(partialShapes.shapes == [pointerShape]
        ...
        suite.expect(emptyShapeTrack.shapes.isEmpty && emptyShapeTrack.shapes.capacity == 0,
        ...
        suite.expect(boundedShapeTrack == shapedTrack
        ...
        suite.expect(plainCanvas == CGSize(width: 960, height: 640),
        ...
        suite.expect(padded.width > 960 && padded.height > 640,
        ...
        let wide = RecorderSupport.canvasSize(source: CGSize(width: 640, height: 640),
                                              ...
                                              aspect: .wide,
                                              cropsToAspect: true)
        suite.expect(abs(wide.width / wide.height - 16.0 / 9.0) < 0.02,
               "a shape preset gives the canvas that shape")
        suite.expect(wide.width <= 640 && wide.height <= 640,
        ...
        let framedWide = RecorderSupport.canvasSize(source: CGSize(width: 640, height: 640),
                                                    padding: 0,
                                                    aspect: .wide)
        suite.expect(framedWide.width > 640,
        ...
        suite.expect(max(huge.width, huge.height) <= RecorderSupport.maximumCanvasEdge,
        ...
        let card = RecorderSupport.cardRect(canvas: CGSize(width: 1200, height: 800),
                                            ...
                                            padding: 0.1)
        suite.expect(abs(card.width / card.height - 960.0 / 640.0) < 0.01,
               "the recording keeps its own proportions inside the canvas")
        suite.expect(abs(card.midX - 600) < 1 && abs(card.midY - 400) < 1,
               "the recording is centred in the canvas")
        suite.expect(card.minX >= 0 && card.maxX <= 1200 && card.minY >= 0 && card.maxY <= 800,
               "the recording never hangs off the canvas")
        let squareCrop = RecorderSupport.cardRect(canvas: CGSize(width: 640, height: 640),
                                                  source: CGSize(width: 960, height: 640),
                                                  padding: 0,
                                                  fillsCanvas: true)
        suite.expect(squareCrop.height == 640 && squareCrop.width == 960 && squareCrop.minX == -160,
        ...
        suite.expect(rotatedGeometry.size == CGSize(width: 1080, height: 1920),
               "an imported rotated video uses its displayed portrait dimensions")
        suite.expect(abs(rotatedRect.minX) < 0.001 && abs(rotatedRect.minY) < 0.001,
               "an imported video transform is normalized to the output origin")
        suite.expect(RecorderSupport.videoGeometry(naturalSize: .zero,
                                             preferredTransform: .identity).size == .zero,
               "missing video dimensions stay invalid instead of becoming an artificial frame")

        suite.expect(RecorderSupport.sanitizedZoomAmount(99) == RecorderSupport.zoomAmountRange.upperBound
        ...
        suite.expect(visibleTrack.samples.count == 2
        ...
        suite.expect(RecorderMotion.resampledVisibility(visibleTrack.samples,
        ...
        suite.expect(studio.zoomEnabled && studio.showsPointer && !studio.backdrop.isEmpty,
               "the studio look turns on everything it promises")
        let bare = RecorderEditDocument().applying(.raw)
        suite.expect(!bare.zoomEnabled && !bare.showsClickRing && bare.backdrop.isEmpty,
               "the raw look leaves the recording exactly as it was")
        suite.expect(bare.showsPointer && bare.resolvedSmoothing == .off,
               "raw still draws the pointer, because the capture itself has none, but eases nothing")
        suite.expect(RecorderEditDocument().applying(.clean).zoomEnabled
                && RecorderEditDocument().applying(.clean).backdrop.isEmpty,
               "the clean look smooths and zooms without putting anything behind it")

        let takeID = UUID()
        suite.expect(RecorderSupport.takeID(fromFolderName: RecorderSupport.takeFolderName(id: takeID))
                == takeID,
               "a recording folder name round-trips its id")
        suite.expect(RecorderSupport.takeID(fromFolderName: "Downloads") == nil,
        ...
        for language in AppLanguage.allCases {
            ...
            suite.expect(!batteryExample.isEmpty && !batteryExample.contains(" "),
        ...
        for language in agreeingLanguages {
            ...
            suite.expect(!processes.hasPrefix("%d"),
        ...
        suite.expect(accented.sorted() == ["Zebra", "Área", "Ímã"],
        ...
        for language in AppLanguage.allCases {
            ...
            suite.expect(CommandBarSearch.matchesVerb(verb, in: format),
                   "the quit verb finds its own rows in \(language.rawValue)")
        }
        suite.expect(CommandBarSearch.matchesVerb("quit saf", in: "Quit %@")
        ...
        suite.expect(!CommandBarSearch.matchesVerb("safari", in: "Quit %@")
                && !CommandBarSearch.matchesVerb("", in: "Quit %@"),
               "an app name alone never drags the quit rows in")
        suite.expect(InstalledApps.isSystemApplication(
        ...
        suite.expect(InstalledApps.isInApplicationsFolder(
