...
enum NotchMusicExtrasTests {
    ...
    static func run(expect: (Bool, String) -> Void) {
        ...
        expect(lyrics.activeIndex(at: 0) == nil && lyrics.activeIndex(at: 12.249) == nil
        ...
        expect(lyrics.activeIndex(at: 19.875, offset: 0.5) == 0
        ...
        expect(lyrics.activeIndex(at: .nan) == nil && lyrics.activeIndex(at: 12, offset: .infinity) == nil,
               "invalid playback timing never highlights a lyric")
        expect(NotchLyricsSupport.parse("[00:02]later\n[00:01]earlier", duration: 3).map(\.text) == ["earlier", "later"],
               "out-of-order lyric entries are sorted before following playback")
        expect(NotchLyricsSupport.parse("[00:01]line\n[offset:-500]", duration: 2).first?.time == 1.5,
               "a negative file offset delays its lyric timestamps")
        expect(NotchLyricsSupport.parse(String(repeating: "x", count: NotchLyricsSupport.maximumBytes + 1), duration: 10).isEmpty
        ...
        expect(NotchLyricsSupport.parse("unmarked words", duration: 20).isEmpty,
        ...
        expect(query.host == "lrclib.net" && query.path == "/api/get"
        ...
        expect(decode()?.lines.count == 1, "an exact recording match can provide synchronized lyrics")
        reply["duration"] = 183.0
        expect(decode() == nil, "a different recording length is not treated as the same synchronized lyric")
        reply["duration"] = 180.0
        reply["albumName"] = "Concert Recording"
        expect(decode() == nil, "a matching title and artist never substitute a different album or live recording")
        reply["albumName"] = identity.album
        reply["trackName"] = identity.title + " (Live)"
        expect(decode() == nil, "version qualifiers are not stripped from the lyric match")
        reply["trackName"] = identity.title
        reply["instrumental"] = true
        expect(decode()?.instrumental == true && decode()?.lines.isEmpty == true,
        ...
        expect(decodeQueue() == nil, "queue data must be anchored to the observed current song")
        queue["currentIdentifier"] = "current-item"
        queue["pid"] = 43
        expect(decodeQueue() == nil, "identical song metadata in a different player cannot inherit a queue action")
        queue["pid"] = 42
        let invalidPIDs: [Any] = [true, 42.5, -1, Double.nan, Double(Int32.max) + 1]
        for invalidPID in invalidPIDs {
            queue["pid"] = invalidPID
            expect(decodeQueue() == nil, "a queue rejects malformed or truncated player identities")
        }
        queue["pid"] = 42
        queue["queueItems"] = [] as [[String: Any]]
        expect(decodeQueue()?.items.isEmpty == true, "a genuinely empty queue differs from an unsupported queue")
        queue["queueAvailable"] = false
        expect(decodeQueue() == nil, "a player without a native queue cannot be represented as an empty playlist")
        for invalidRows in [
            ...
            expect(decodeQueue() == nil, "ambiguous or invalid native queue identities fail closed")
        }
        ...
        expect(NotchPlaybackCommand.queuePlay(selection(String(repeating: "x", count: 513))).message == nil
        ...
        expect(NotchLyricsSupport.onlineEnabled(in: defaults), "online lyrics require a separate explicit choice")
