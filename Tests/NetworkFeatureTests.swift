...
enum NetworkFeatureTests {
    static func run(_ suite: TestSuite) {
        ...
        suite.expectClose(speed.down, 1024, "down speed over 2s")
        suite.expectClose(speed.up, 512, "up speed over 2s")

        let zeroElapsed = MetricFormat.netSpeed(previous: slow, current: fast, elapsed: 0)
        suite.expect(zeroElapsed.down == 0 && zeroElapsed.up == 0, "zero elapsed yields zero")
        ...
        suite.expect(afterReset.down == 0 && afterReset.up == 0, "counter reset yields zero")
        ...
        suite.expect(firstFrozenDownload.sampleProcesses && !firstFrozenDownload.useProcessDownload,
        ...
        suite.expect(secondFrozenDownload.sampleProcesses && secondFrozenDownload.useProcessDownload,
        ...
        suite.expect(fallbackIdle.sampleProcesses && fallbackIdle.useProcessDownload,
        ...
        suite.expect(!inboundRecovered.sampleProcesses && !inboundRecovered.useProcessDownload,
        ...
        suite.expect(!counterResetFallback.sampleProcesses && !counterResetFallback.useProcessDownload,
        ...
        suite.expect(fallbackBaseline.downBytesPerSec == nil && fallbackBaseline.upBytesPerSec == nil,
               "network sampler first reading remains a baseline")
        suite.expect(fallbackProbe.downBytesPerSec == 0 && fallbackProbe.upBytesPerSec == 100,
               "first frozen inbound sample keeps interface output while priming fallback")
        suite.expectClose(fallbackReading.downBytesPerSec ?? -1, 2_000,
                    "network sampler replaces frozen interface download with process traffic")
        suite.expectClose(fallbackReading.upBytesPerSec ?? -1, 100,
                    "network sampler keeps the lightweight interface upload counter")
        suite.expect(fallbackReading.totalDown == 4_000 && fallbackReading.totalUp == 400,
               "network fallback accumulates process download and interface upload totals")
        suite.expectClose(recoveredReading.downBytesPerSec ?? -1, 100,
                    "network sampler returns to interface download after counter recovery")
        suite.expect(recoveredReading.totalDown == 4_200 && recoveredReading.totalUp == 500,
        ...
        suite.expect(nettopRows.count == 2,
               "nettop parser keeps only active rows from the final delta section")
        suite.expect(nettopRows.first?.name == "Codex (Service)" && nettopRows.first?.pid == 78844,
        ...
        suite.expect(streamedSections.count == 1,
               "nettop stream parser skips the initial cumulative section")
        suite.expect(streamedSections.first?.count == 1,
               "nettop stream parser emits only active rows from the first delta section")
        suite.expectClose(streamedSections.first?.first?.bytesIn ?? -1, 416,
                    "nettop stream parser does not publish cumulative bytes")
        suite.expect(NetworkProcessSupport.nettopArguments == ["-P", "-d", "-x", "-J", "bytes_in,bytes_out", "-L", "1", "-s", "1"],
               "nettop per-app sampling asks for one cumulative section and computes deltas in app")
        suite.expect(NetworkProcessSupport.externalNettopArguments == ["-P", "-d", "-x", "-t", "external", "-J", "bytes_in,bytes_out", "-L", "1", "-s", "1"],
        ...
        suite.expect(cappedProcess.status == 0 && cappedProcess.output.count == 1_024,
        ...
        suite.expect(orphanedPipe.status == 0
        ...
        suite.expect(timedOutProcess.timedOut && timedOutProcess.status == -1
        ...
        suite.expect(networkDelta.rates(from: baselineNetwork, now: 100).isEmpty,
        ...
        suite.expect(rateNetwork.count == 1 && rateNetwork.first?.pid == 10,
               "network process delta keeps only processes with traffic")
        suite.expectClose(rateNetwork.first?.bytesIn ?? -1, 1_024,
                    "network process delta computes download bytes per second")
        suite.expectClose(rateNetwork.first?.bytesOut ?? -1, 512,
        ...
        suite.expect(resetNetwork.isEmpty,
        ...
        suite.expect(staleNetwork.isEmpty,
        ...
        suite.expect(NetworkProcessSamplingPolicy.leaseIsActive(expiresAt: renewedLease, now: 61.9),
               "network monitoring lease remains active before expiry")
        suite.expect(!NetworkProcessSamplingPolicy.leaseIsActive(expiresAt: renewedLease, now: 62.0),
               "network monitoring lease expires exactly at the boundary")
        suite.expectClose(NetworkProcessSamplingPolicy.shortenedLease(currentExpiresAt: 90, now: 50),
                    54,
                    "network monitoring stop shortens the lease instead of depending on balanced disappear events")

        suite.expect(MonitorSamplingPolicy.sampleStride(for: .cpu, intervalSeconds: 2, foreground: false) == 1,
               "monitor CPU stays responsive in menu-bar-only mode")
        suite.expect(MonitorSamplingPolicy.sampleStride(for: .disk, intervalSeconds: 2, foreground: false) == 5,
               "monitor disk sampling slows down in menu-bar-only mode without exceeding DiskSampler.maxGap")
        suite.expect(MonitorSamplingPolicy.sampleStride(for: .peripheralBattery, intervalSeconds: 2, foreground: false) == 30,
               "monitor peripheral battery sampling is heavily throttled in menu-bar-only mode")
        suite.expect(MonitorSamplingPolicy.sampleStride(for: .fanSpeed, intervalSeconds: 2, foreground: false) == 3,
               "fan speed refreshes without waking the monitor every base tick")
        suite.expect(MonitorSamplingPolicy.sampleStride(for: .disk, intervalSeconds: 2, foreground: true) == 1,
               "monitor disk sampling stays live while the panel is open")
        suite.expect(MonitorSamplingPolicy.shouldSample(.disk, tick: 4, intervalSeconds: 2, foreground: false) == false,
               "monitor skips heavy menu-bar-only ticks before the stride")
        suite.expect(MonitorSamplingPolicy.shouldSample(.disk, tick: 5, intervalSeconds: 2, foreground: false),
               "monitor samples heavy menu-bar-only ticks at the stride")

        suite.expect(MonitorSamplingPolicy.wakeTicks(for: [.cpu, .disk], intervalSeconds: 2, foreground: false) == 1,
               "monitor wakes every tick while an every-tick metric is on")
        suite.expect(MonitorSamplingPolicy.wakeTicks(for: [.temperature], intervalSeconds: 2, foreground: false) == 8,
               "monitor with only temperature wakes once per temperature stride")
        suite.expect(MonitorSamplingPolicy.wakeTicks(for: [.fanSpeed], intervalSeconds: 2, foreground: false) == 3,
               "monitor with only fan speed wakes once per fan stride")
        suite.expect(MonitorSamplingPolicy.wakeTicks(for: [.peripheralBattery], intervalSeconds: 2, foreground: false) == 30,
               "monitor with only peripheral battery wakes once per minute")
        suite.expect(MonitorSamplingPolicy.wakeTicks(for: [.disk, .peripheralBattery], intervalSeconds: 2, foreground: false) == 5,
               "monitor wake cadence is the GCD of the needed strides")
        suite.expect(MonitorSamplingPolicy.wakeTicks(for: [.temperature], intervalSeconds: 2, foreground: true) == 1,
        ...
        suite.expect(MonitorSamplingPolicy.alignedTick(9, wakeTicks: 1) == 9,
               "monitor tick needs no alignment at every-tick cadence")

        // MARK: Interface filtering

        suite.expect(MetricFormat.includeNetworkInterface("en0"), "en0 included")
        suite.expect(MetricFormat.includeNetworkInterface("en12"), "en12 included")
        suite.expect(!MetricFormat.includeNetworkInterface("lo0"), "lo0 excluded")
        suite.expect(!MetricFormat.includeNetworkInterface("awdl0"), "awdl0 excluded")
        suite.expect(!MetricFormat.includeNetworkInterface("nan0"), "nan0 excluded")
        suite.expect(!MetricFormat.includeNetworkInterface("utun3"), "utun3 (VPN) excluded")
        suite.expect(!MetricFormat.includeNetworkInterface("bridge0"), "bridge0 excluded")
        suite.expect(!MetricFormat.includeNetworkInterface(""), "empty excluded")
        ...
        history.push(1)
        history.push(2)
        suite.expect(history.values == [1, 2], "history keeps order under capacity")
        history.push(3)
        history.push(4)
        suite.expect(history.values == [2, 3, 4], "history drops oldest at capacity")
        suite.expect(history.values.count == 3, "history never exceeds capacity")
        suite.expect(history.publishedValues(whileVisible: false).isEmpty
