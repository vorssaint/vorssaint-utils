...
enum NotchTests {
    ...
    static func run(_ suite: TestSuite) {
        ...
        suite.expect(homeGeometry.expandedSize(module: .controls, controlsHaveMusic: true).height
        ...
        suite.expect(NotchSupport.modules(in: defaults) == [.system, .music, .controls, .mixer, .captures, .files, .tools, .calendar, .timer, .downloads],
        ...
        suite.expect(compact.sectionPickerSize(count: 1, searching: true).height < compact.sectionPickerSize(count: allModules.count).height,
               "a short search result shrinks the gallery instead of reserving empty rows")
        suite.expect(compact.contentSize(for: compact.sectionPickerSize(count: 0, searching: true)).height >= 160,
        ...
        suite.expect(fullControls.height > fewerShortcuts.height && fewerShortcuts.height > onlyShortcuts.height,
               "hiding shortcuts or sliders removes their unused vertical space")
        suite.expect(compact.contentSize(for: compact.expandedSize(module: .controls, controlRows: 0, sliderCount: 0)).height >= 160,
        ...
        for width in [360.0, 480.0, 600.0] {
            ...
            for shortcuts in 1...8 {
                ...
                for sliders in 0...2 {
                    ...
                    suite.expect(geometry.contentSize(for: size).height == min(requiredHeight, availableHeight),
        ...
        for frame in frames {
            for notch in [false, true] {
                ...
                suite.expect(geometry.contentSize(for: geometry.expandedSize(module: .music)).height >= 212,
        ...
        for layout in NotchSize.allCases {
            for customHeight in [400.0, 640.0] {
                ...
                suite.expect(geometry.contentSize(for: preview).height == 214 && preview.height < history.height,
