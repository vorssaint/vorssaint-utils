...
5. **Video Downloader, only after you paste a link.** Inspecting and downloading media connects directly to the site hosting the video from your Mac, through local `yt-dlp` and `ffmpeg` tools. Preview thumbnails load directly from the source site. Vorssaint never sends your links, titles or files to any Vorssaint server, never keeps a history of what you downloaded, and never writes links to disk. Links are passed to `yt-dlp` over standard input so they never appear in your Mac's process list.

The downloader isolates `yt-dlp` from your system dotfiles: it ignores custom plugins, shell hooks and global configuration files. If you turn on browser cookies for member-only or age-restricted videos, `yt-dlp` reads session cookies directly from the browser you choose during that download; Vorssaint never stores or transmits them. Installing missing tools (`yt-dlp`, `ffmpeg` or `deno`) runs through Homebrew and talks only to official package repositories.
