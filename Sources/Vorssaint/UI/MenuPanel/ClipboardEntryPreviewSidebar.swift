            ...
            if let name = entry.imageFile,
               let image = ClipboardImageStore.thumbnail(named: name) {
                Image(nsImage: image)
                    ...
                    .modifier(PreviewDraggable(entry: entry))
            }
        ...
        VStack(alignment: .leading, spacing: 10) {
            if isImage, let thumbnail {
                Image(nsImage: thumbnail)
                    ...
                    .frame(maxWidth: .infinity)
                    .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))
                    .modifier(PreviewDraggable(path: path))
...
/// The preview's thumbnail or file icon drags out as the file, the same way
/// the row does. A press that never drags does nothing here: the preview has
/// no click action of its own to hand it to.
private struct PreviewDraggable: ViewModifier {
    let items: [ClipboardHistoryService.DragItem]

    init(entry: ClipboardHistoryEntry) {
        items = ClipboardHistoryService.dragItems(for: entry)
    }

    init(path: String) {
        let url = URL(fileURLWithPath: path)
        items = FileManager.default.fileExists(atPath: path)
            ? [ClipboardHistoryService.DragItem(writer: url as NSURL,
    ...
    func body(content: Content) -> some View {
        if items.isEmpty {
            ...
            content.overlay(ClipboardFileDragSource(items: items) { _ in })
        }
    }
}
