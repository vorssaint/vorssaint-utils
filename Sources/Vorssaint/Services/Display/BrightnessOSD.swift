...
enum BrightnessOSD {
    ...
    static func show(displayID: CGDirectDisplayID, brightness: Double) {
        ...
        if let existing = Self.host {
            existing.rootView = AnyView(BrightnessOSDView(brightness: brightness).localizedLayoutDirection())
            host = existing
        } else {
            host = NSHostingController(rootView: AnyView(BrightnessOSDView(brightness: brightness).localizedLayoutDirection()))
            Self.host = host
