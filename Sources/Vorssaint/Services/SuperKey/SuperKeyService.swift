...
final class SuperKeyService: ObservableObject {
    ...
    private func performMapping(_ enabled: Bool,
        ...
        // the cross-keyboard comparison, carried along in the write — but only
        // while their marker says a confirmed write made them. An entry of
        // that shape with no marker behind it is external and stays the user's.
        ...
        guard let existing = SuperKeySupport.consistentMappings(
            ...
            propagating: partnerApplied ? KeyOverrideSupport.isOwnedMapping : { _ in false }
