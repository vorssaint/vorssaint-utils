...
final class VideoDownloaderWorkflow: ObservableObject {
    ...
    /// The debounce has accepted a valid URL and will inspect it as soon as it runs.
    /// Keeping this separate from `.inspecting` lets the UI retain an existing result
    /// footprint instead of collapsing it during that short delay.
    var isInspectionPending: Bool {
        pendingValidatedSource != nil
            && dependencyState.value?.isReady == true
            && [.idle, .inspecting].contains(phase)
    }
