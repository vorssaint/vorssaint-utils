...
final class SecureInputMonitor: ObservableObject {
    ...
    /// any page's onAppear/onDisappear: those do not always run when the
    /// window closes, and do not run at all when the window reopens on a
    /// page that was never removed from the view hierarchy, so `demand`
    /// alone cannot tell whether anything is actually on screen.
    ...
    func setSettingsWindowOpen(_ open: Bool) {
        ...
        isWindowOpen = open
        if open {
            if !observingSurfaces.isEmpty { refresh() }
        } else {
            // Clearing without a read: a stale `.app(X)` would keep warning
            // about a holder nobody can see while the window is closed.
            publish(.off)
        }
        schedulePolling()
    }
