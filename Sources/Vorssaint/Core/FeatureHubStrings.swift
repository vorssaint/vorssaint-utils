...
extension FeatureHubStrings {
    static let ko = FeatureHubStrings(
        ...
        explainFullDisk: "정리기와 앱 제거기가 어디서나 남은 파일을 찾고, 비디오 다운로더가 켜진 경우 브라우저 쿠키를 읽을 수 있게 합니다.",
        explainFilesAndFolders: "비디오 다운로더가 보호된 폴더에 저장하고 WhatsApp 다운로드 정리 및 실험적 구성 기능이 다운로드 폴더를 확인할 수 있게 합니다.",
...
extension FeatureHubStrings {
    static let enUS = FeatureHubStrings(
        ...
        explainFullDisk: "Lets the cleaner and the uninstaller find leftover files everywhere, and lets the video downloader read browser cookies when enabled.",
        explainFilesAndFolders: "Allows the video downloader to save to protected folders and WhatsApp cleanup and its experimental organizer to inspect Downloads.",
    ...
    static let ptBR = FeatureHubStrings(
        ...
        explainFullDisk: "Deixa o limpador e o desinstalador acharem restos de arquivos em todo lugar e permite que o baixador de vídeos leia cookies do navegador quando ativado.",
        explainFilesAndFolders: "Permite que o baixador de vídeos salve em pastas protegidas e que a limpeza e o organizador experimental do WhatsApp verifiquem Downloads.",
    ...
    static let tr = FeatureHubStrings(
        ...
        explainFullDisk: "Temizleyicinin ve kaldırıcının artık dosyaları her yerde bulmasını; video indiricinin de etkinleştirildiğinde tarayıcı çerezlerini okumasını sağlar.",
        explainFilesAndFolders: "Video indiricinin korunan klasörlere kaydetmesini, WhatsApp temizliğinin ve deneysel düzenleyicinin İndirilenler'i denetlemesini sağlar.",
    ...
    static let ru = FeatureHubStrings(
        ...
        explainFullDisk: "Позволяет очистке и деинсталлятору находить остатки файлов повсюду, а загрузчику видео читать cookies браузера, если эта функция включена.",
        explainFilesAndFolders: "Разрешает загрузчику видео сохранять в защищённые папки, а очистке и экспериментальному органайзеру WhatsApp проверять «Загрузки».",
    ...
    static let es = FeatureHubStrings(
        ...
        explainFullDisk: "Permite que el limpiador y el desinstalador encuentren restos en todas partes, y que el descargador de vídeos lea las cookies del navegador cuando se activa.",
        explainFilesAndFolders: "Permite que el descargador de vídeos guarde en carpetas protegidas y que la limpieza y el organizador experimental de WhatsApp examinen Descargas.",
    ...
    static let de = FeatureHubStrings(
        ...
        explainFullDisk: "Lässt Reiniger und Deinstallierer überall Dateireste finden und den Video-Downloader Browser-Cookies lesen, wenn die Funktion aktiviert ist.",
        explainFilesAndFolders: "Erlaubt dem Video-Downloader, in geschützten Ordnern zu speichern, und der WhatsApp-Bereinigung samt experimentellem Organizer, Downloads zu prüfen.",
    ...
    static let fr = FeatureHubStrings(
        ...
        explainFullDisk: "Permet au nettoyeur et au désinstalleur de trouver les restes partout, et au téléchargeur vidéo de lire les cookies du navigateur lorsque l’option est activée.",
        explainFilesAndFolders: "Autorise le téléchargeur vidéo à enregistrer dans des dossiers protégés, et le nettoyage WhatsApp avec son organisateur expérimental à examiner Téléchargements.",
    ...
    static let it = FeatureHubStrings(
        ...
        explainFullDisk: "Permette a pulizia e disinstallazione di trovare residui ovunque e al downloader video di leggere i cookie del browser quando l'opzione è attiva.",
        explainFilesAndFolders: "Consente allo strumento per scaricare video di salvare nelle cartelle protette e alla pulizia WhatsApp con l'organizzatore sperimentale di controllare Download.",
    ...
    static let ja = FeatureHubStrings(
        ...
        explainFullDisk: "クリーナーとアンインストーラが残りファイルをどこでも見つけ、ビデオダウンローダーが有効時にブラウザのCookieを読めるようにします。",
        explainFilesAndFolders: "ビデオダウンローダーが保護されたフォルダに保存し、WhatsAppのダウンロード整理と実験的オーガナイザーがダウンロードフォルダを確認できるようにします。",
    ...
    static let zhHans = FeatureHubStrings(
        pageTitle: "功能",
        intro: "只安装你会用到的功能。卸载的功能会从整个应用中消失，不再加载。",
        tabFeatures: "功能",
        tabPermissions: "权限",
        ...
        permAudioCapture: "应用音频",
        explainAccessibility: "让功能响应点按和按键，并移动窗口。",
        explainScreenRecording: "让功能显示窗口缩略图并读取屏幕上的文字。",
        explainFullDisk: "让清理器和卸载器在任何位置找到残留文件，并让视频下载器在启用后读取浏览器 Cookie。",
        explainFilesAndFolders: "允许视频下载器保存到受保护的文件夹，并允许 WhatsApp 下载清理和实验性整理器检查下载文件夹。",
        explainNotifications: "让应用就你开启的警报发出通知。",
        explainAutomationFinder: "让应用请访达替你移动文件。",
        explainAutomationTerminal: "让 Homebrew 命令在终端中打开。",
        explainAudioCapture: "让混音器调整每个应用的音量。",
        descSwitcher: "带预览地切换应用和窗口",
    ...
    static let zhTW = FeatureHubStrings(
        ...
        explainFullDisk: "讓清理器和解除安裝器在任何位置找到殘留檔案，並讓影片下載器在啟用後讀取瀏覽器 Cookie。",
        explainFilesAndFolders: "允許影片下載器儲存到受保護的資料夾，並允許 WhatsApp 下載項目清理和實驗性整理器檢查下載項目資料夾。",
    ...
    static let zhHK = FeatureHubStrings(
        ...
        explainAccessibility: "讓功能回應點按和按鍵，並移動視窗。",
        explainScreenRecording: "讓功能顯示視窗縮圖並讀取螢幕上的文字。",
        explainFullDisk: "讓清理器和移除器在任何位置找到殘留檔案，並讓影片下載器在啟用後讀取瀏覽器 Cookie。",
        explainFilesAndFolders: "允許影片下載器儲存到受保護的資料夾，並允許 WhatsApp 下載項目清理和實驗性整理器檢查下載項目資料夾。",
        explainNotifications: "讓 App 就你開啟的警示發出通知。",
