...
- **Charge limit.** Set a charging ceiling (20–100%) so a plugged-in MacBook stops at the level you choose, temporarily top up to 100%, switch to battery power until stopped, or run a calibration cycle (100% → 10% → 100%, hold an hour, then return to the limit). On macOS 27, native limits use 80–100% in 5% steps. Off by default in Features; uses a protected helper.
