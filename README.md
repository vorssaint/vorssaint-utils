...
- **Music app blocker.** Stops the Music app from opening after a detected playback button press. Requires Accessibility; uncertain launches stay open, including headphone connections that send no playback command.
...
- **Network.** Live rates, session totals, the Mac's local IP address and a built in speed test.
...
- **Dock Preview.** Hover a Dock icon to see adjustable window thumbnails with clear titles from all desktops by default. Click one to switch to its desktop or drag it to move and snap the window. Turn on “Show only the current desktop” to limit previews. Middle-click closes only the pointed window, including in pinned previews. Optional minimal previews hide titles, controls and decoration. An experimental option under “More options” keeps the Dock visible until you leave or press a key; it may resize windows.
